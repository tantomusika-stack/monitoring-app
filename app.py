            ...
            if laporan_data:
                for rep in laporan_data:
                    with st.expander(f"📄 {rep['cabor']} - {rep['tanggal_kegiatan']}"):
                        ...
                        file_name = rep.get("file_name", "laporan.docx")
                        
                        if file_path:
                            try:
                                # Tampilkan tombol download dengan mengambil data dari bucket
                                file_data = supabase.storage.from_("laporan-monev").download(file_path)
                                st.download_button(
                                    label=f"📥 Unduh Laporan Ini",
                                    data=file_data,
                                    file_name=file_name,
                                    mime="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                                    key=f"dl_btn_{rep['id']}" 
                                )
                            except Exception as e:
                                st.error("Gagal mengambil file dari server (Pastikan Storage Policy untuk Select/Read sudah aktif).")
                        else:
                            st.warning("Path file tidak ditemukan di database.")
            else:
                st.info("Belum ada laporan yang tersimpan.")
        except Exception as e:
            st.info(f"Terjadi kesalahan saat memuat arsip: {e}")
